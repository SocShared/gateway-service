...
public class VkController implements VkApi {
    ...
    public Page<VkAdapterGroupResponse> getGroupUserList(@Param("page") Integer page,
                                                         @Param("size") Integer size,
                                                         @RequestHeader(value = "Authorization", required = false) String token) {
        UUID systemUserId = UUID.fromString("75853635-c91a-492b-9276-2b7d3511e0de");
        token = "RequiredToken!";
        PageAdapter<VkAdapterGroupResponse> res = client.getGroups(systemUserId, page, size, token);
        Page<VkAdapterGroupResponse> pageOfGroup = new PageImpl<>(res.getObject());
        return pageOfGroup;
